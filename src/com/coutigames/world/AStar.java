...
public class AStar {
	...
	public static List<Node> findPath(World world, Vector2i start, Vector2i end) {
		...
		while (openList.size() > 0) {
			...
			if (current.tile.equals(end)) {// final do destino
			...
			for (int i = 0; i < 9; i++) {
				...
				if (vecIntList(closedList, a) && gcost >= current.gcost)
					continue;
				if (!vecIntList(openList, a)) {
					openList.add(node);
	...
	private static boolean vecIntList(List<Node> list, Vector2i vector) {
